...
@register_env("AlignFr3Env-v1", max_episode_steps=100, asset_download_ids=["ycb"])
class AlignFr3Env(RealFr3):
    ...
    def _load_scene(self, options: dict):
        ...
        # Build green can using DTC mesh 
        ...
        self.green = builder.build(name=f"dtc_green_can_fr3")
        ...
        builder.initial_pose = sapien.Pose(p=[self.x_offset, -0.2, 0])
        builder.set_scene_idxs(list(range(self.num_envs)))
        self.red = builder.build(name=f"dtc_red_tomato_can_fr3")
        
        self._objs = [self.green, self.red]

        self.goal_site = actors.build_box(
                    self.scene,
                    half_sizes=[0.0725 * 2.54 * 0.5, 0.11625 * 2.54 * 0.5, 0.05375 * 2.54 * 0.5],
                    color=[0, 1, 0, 1],
                    name="spice_rack",
                    initial_pose=sapien.Pose([self.x_offset, -0.2, 0]),
                )

        self.goal_height = 0.068
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            green_can_rotate_fix = quaternion_multiply(
            ...
            rack_rotate_change = euler2quat(0, 0, -math.pi/2)

            b = len(env_idx)
            self.table_scene.initialize(env_idx)
            # add noise
            obj_xyz_0 = torch.zeros((b, 3))
            obj_xyz_0[:, 0] = -0.2 + torch.rand(b) * 0.05 + self.x_offset #  -0.25 to -0.15
            obj_xyz_0[:, 1] = 0.1 + abs(torch.rand((b))) * 0.1        # 0.1, 0.2
            obj_xyz_0[:, 2] = 0.0
            obj_qs_0 = random_quaternions(b, lock_x=True, lock_y=True)
 
            obj_xyz_1 = torch.zeros((b, 3))
            obj_xyz_1[:, 0] = obj_xyz_0[:, 0] + abs(torch.rand((b))) * 0.05 + 0.1 # - 0.2 to -0.1
            obj_xyz_1[:, 1] = torch.rand((b)) * 0.1 + 0.1 # 0.1, 0.2
            obj_xyz_1[:, 2] = 0.04

            goal_xyz_0 = torch.zeros((b, 3))
            goal_xyz_0[:, 0] = torch.rand((b)) * 0.2 - 0.25 + self.x_offset # 0.05, -0.15 / + self.x_offset
            goal_xyz_0[:, 1] = torch.rand((b)) * 0.1 - 0.2 # -0.2, -0.1
            goal_xyz_0[:, 2] = self.goal_height # self.goal_height # * 0.5 # put it on the table, hard code from human render mode

            while torch.norm(obj_xyz_0 - obj_xyz_1, dim=1) < 0.1 or torch.norm(obj_xyz_0 - goal_xyz_0, dim=1) < 0.15:
                obj_xyz_1[:, 0] = torch.rand((b)) * 0.2 - 0.25 + self.x_offset # 0.05, -0.15 / + self.x_offset
                obj_xyz_1[:, 1] = torch.rand((b)) * 0.1 + 0.1 # 0.1, 0.2

            # NOTE: haoran: only consider num_env = 1
            ...
            self._objs[0].set_pose(Pose.create_from_pq(p=obj_xyz_0, q=green_can_rotate_fix)) 
            self._objs[1].set_pose(Pose.create_from_pq(p=obj_xyz_1, q=red_can_rotate_fix)) 
            self.goal_site.set_pose(Pose.create_from_pq(p=goal_xyz_0, q=rack_rotate_change))
    ...
    def evaluate(self):
        ...
        xy_offset_1 = obj_pos_1[..., :2] - goal_box_pos[..., :2]
        ...
        is_xy0_inside = torch.linalg.norm(xy_offset_0, axis=1) <= half_xy_goal - 0.02  # Allow some margin
        is_xy1_inside = torch.linalg.norm(xy_offset_1, axis=1) <= half_xy_goal - 0.02  # Allow some margin
        
        # Object is inside box if both xy and z conditions are met
        is_obj_in_box = torch.logical_and(is_xy0_inside, is_xy1_inside) # we dont check z axis
        
        # Check if robot is grasping the object
        is_grasped_0 = self.agent.is_grasping(self._objs[0])
        is_grasped_1 = self.agent.is_grasping(self._objs[1])
        is_grasped = torch.logical_and(is_grasped_0, is_grasped_1)
        ...
        is_obj_1_static = torch.logical_and(
            torch.linalg.norm(self._objs[1].linear_velocity, axis=1) < 0.05,
            torch.linalg.norm(self._objs[1].angular_velocity, axis=1) < 0.5
        )
        is_obj_static = torch.logical_and(is_obj_0_static, is_obj_1_static)
        ...
        return {
            ...
            "is_obj0_in_box": is_xy0_inside,
            "is_obj1_in_box": is_xy1_inside,
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        is_grasped = torch.logical_and(info["is_grasped_0"], info["is_grasped_1"]) # TODO fix bug here, not called?
