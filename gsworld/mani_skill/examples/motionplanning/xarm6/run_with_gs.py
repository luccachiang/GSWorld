...
from gsworld.mani_skill.examples.motionplanning.xarm6.solutions import *
...
MP_SOLUTIONS = {
    "AlignXArmEnv-v1": SolveAlignXArmEnv,
    "SpoonOnBoardXArmEnv-v1": SolveSpoonOnBoardXArmEnv,
    "BananaRotationXArmEnv-v1": SolveRotateBananaXArmEnv
}
...
def parse_args(args=None):
    ...
    parser.add_argument("-e", "--env-id", type=str, default="PickALignYCBXArmEnv-v1", help=f"Environment to run motion planning solver on. Available options are {list(MP_SOLUTIONS.keys())}")
    ...
    parser.add_argument("--scene_cfg_name", default="xarm_table_pour", type=str, help="Path to scene json config file")
...
def _main(args, pipeline, proc_id: int = 0, start_seed: int = 0) -> str:
    ...
    env = GSWorldWrapper(
        ...
        robot_pipe=pipeline.extract(args),
        device="cuda",
    )
...
def main(args, pipeline):
    ...
    save_images_to_mp4(hdf5_path=output_path, output_dir=output_path[: -len(".h5")], save_frame=False)
