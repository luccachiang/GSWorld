...
class XArmMotionPlanningSolver:
    def __init__(
        self,
        env: BaseEnv,
        debug: bool = False,
        ...
        if self.vis and self.visualize_target_grasp_pose:
            if "grasp_pose_visual" not in self.base_env.scene.actors:
                self.grasp_pose_visual = build_xarm_gripper_grasp_pose_visual(
                    self.base_env.scene
                )
            else:
...
def build_xarm_gripper_grasp_pose_visual(scene: ManiSkillScene):
