...
class GaussianPlannningRunner:
    ...
    def _create_parser(self):
        ...
        parser.add_argument("-e", "--env-id", type=str, default="PickCube-v1", help="Environment to run motion planning solver on")
        parser.add_argument("-n", "--num-traj", type=int, default=10, help="Number of trajectories to generate.")
        parser.add_argument("-r", "--robot-uid", type=str, default="fr3_umi", help="Robot uid.")
    ...
    def run(self, args=None):
        ...
        from gsworld.mani_skill.examples.motionplanning.fr3_umi.run_with_gs import main
...
if __name__ == "__main__":
    ...
    config = {
        "env_id": "PnpBox-v1",
        "scene_cfg_name": "fr3_pnp_box", # fr3_pnp_box0411
        "num_traj": 1,
        "sim_backend": "cpu",
        # "device": "cuda:2", # TODO if single gpu dagger is enough, fine. if not, consider change gpu
        "save_video": True,
        "only_count_success": False,
        "record_dir": os.path.join(os.path.dirname(os.path.abspath(__file__)), "exp_log/teaser_rendering"),
        "num_procs": 1, # ~20cpu/procs
        "obs_mode": "rgb",
        "ep_len": 800,
        "vis": True,
    }
