...
class FR3UmiMotionPlanningSolver:
    def __init__(
        self,
        env: BaseEnv,
        debug: bool = False,
        ...
        if self.vis and self.visualize_target_grasp_pose:
            if "grasp_pose_visual" not in self.base_env.scene.actors:
                self.grasp_pose_visual = build_fr3_gripper_grasp_pose_visual(
                    self.base_env.scene
                )
            else:
    ...
    def setup_planner(self):
        ...
        # gq: note that mplib version is 0.1.1
        planner = mplib.Planner( # gq:srdf can be "" but not none due to mplib version
            urdf=self.env_agent.urdf_path,
            srdf=self.env_agent.urdf_path.replace(".urdf", "_mplib.srdf"), 
    ...
    def follow_path(self, result, refine_steps: int = 0):
        ...
        for i in range(n_step + refine_steps):
            ...
            # TODO solve infinite problem
            if truncated: # TODO if exceed time limit, return; do not add terminated here since success means terminated
                return -1
...
def build_fr3_gripper_grasp_pose_visual(scene: ManiSkillScene):
