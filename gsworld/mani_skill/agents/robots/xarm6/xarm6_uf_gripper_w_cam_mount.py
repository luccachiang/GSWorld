...
@register_agent()
class XArm6UfGripperWrist435(BaseAgent):
    uid = "xarm6_uf_gripper_wrist435"
    urdf_path = f"{FILE_DIR}/mani_skill/assets/robots/xarm6/xarm6_description/xarm6_uf_gripper_w_tcp_d435.urdf"
    urdf_config = dict(
        _materials=dict(
            gripper=dict(
                static_friction=20000.0, dynamic_friction=20000.0, restitution=0.0
            )  # TODO useless?
        ),
        link=dict(
    ...
    arm_stiffness = 1000  # 10000
    arm_damping = 200  # 1000
    arm_friction = 2
    arm_force_limit = 100
    ...
    def _after_loading_articulation(self):
        ...
        # TODO maybe disable add camera here?
        gripper_links = [
            "link6",
    ...
    def _after_init(self):
        ...
        # self.gripper_wid = torch.linalg.norm(self.finger1_link.pose.p - self.finger2_link.pose.p)

    def is_grasping(self, object: Actor, min_force=0.5, max_angle=89):
        ...
        lflag = torch.logical_and(
            lforce >= min_force,
            torch.min(torch.rad2deg(langle), torch.rad2deg(langle) - 90) <= max_angle,
        )
        rflag = torch.logical_and(
            rforce >= min_force,
            torch.min(torch.rad2deg(rangle), torch.rad2deg(rangle) - 90) <= max_angle,
        )

        # print(f"lforce: {lforce}, langle: {torch.rad2deg(langle)}")
        # print(f"rforce: {rforce}, rangle: {torch.rad2deg(rangle)}")
        # print(f"lflag: {lflag}, rflag: {rflag}")
